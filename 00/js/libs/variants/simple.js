(function (shoptet) {
  ...
  function switcher($variant) {
    ...
    shoptet.tracking.trackProducts($form[0], $variant.data('codeid'), 'ViewContent', [
      shoptet.tracking.trackFacebookPixel,
    ]);
    if (trackGA) {
      shoptet.tracking.trackProducts($form[0], $variant.data('codeid'), 'detail', [
    ...
    if (typeof bigImageUrl !== 'undefined') {
      ...
      shoptet.products.replaceImage(bigImageUrl, $variant.attr('data-' + replaceInfo.format), replaceInfo.link);
    }
    ...
    if (typeof variantIndex !== 'undefined') {
      ...
      if (variantIndex == 0) {
        $('.p-detail-inner .default-variant, .p-code .default-variant').removeClass(
        ...
        $('.p-detail-inner .choose-variant.' + variantIndex + ', .p-code .choose-variant.' + variantIndex).removeClass(
          shoptet.variantsCommon.noDisplayClasses
        );
        ...
        if (shoptet.abilities.about.generation > 2) {
          shoptet.quantityDiscounts.onVariantChange(
            !$variant.attr('data-quantity-discount-disabled'),
            Number($variant.attr('data-customerprice')),
            Number($variant.attr('data-min'))
          );
        }
      }
    }
  ...
  shoptet.scripts.libs.variantsSimple = ['handler', 'switcher'];
