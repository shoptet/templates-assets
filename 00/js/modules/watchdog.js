(function (shoptet) {
  function initWatchdog() {
    var hasPromotion, actualPrice, onStock;
    ...
    $('#watchdog-form').on('submit', function () {
      ...
      var consents = $form
        .find("input:checkbox[name='consents\\[\\]']:checked, input:hidden[name='consents\\[\\]']")
        .map(function () {
          return this.value;
        })
        .toArray();
    ...
    function watchdogDataHandler() {
      ...
      if ($simpleHolder.data('simpleholder') == 'TRUE') {
        ...
        var selectedVariantIndex = $('#watchdog-simple-variants-select')
          .find('option[data-codeid="' + watchdogCodeId + '"]')
          .data('index');
        $simpleVariants
          .find("option[data-index='" + (selectedVariantIndex > 0 ? selectedVariantIndex : 1) + "']")
          .prop('selected', true);
