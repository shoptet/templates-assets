(function (shoptet) {
  ...
  function getQuantityDiscountElementByAmount(amount) {
    ...
    amount = Math.max(
      ...Array.from(quantityDiscountsItems)
        .map(item => parseInt(item.dataset.amount))
  ...
  function updateQuantityDiscount(el) {
    ...
    document
      .querySelector('.quantity-discounts__item--highlighted')
      ?.classList.remove('quantity-discounts__item--highlighted');
