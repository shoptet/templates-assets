...
var validate = function (isSubmit) {
  ...
  if (isValid) {
    ...
    if (typeof shoptet.validator.message !== 'undefined') {
      shoptet.validator.showValidatorMessage($(this), shoptet.validator.message, 'msg-error');
    }
  ...
  init: function (currentForm, settings) {
    ...
    $currentForm.submit(function (event) {
      ...
      if (invalidElementsCounter) {
        ...
        if (invalid) {
          var $firstInvalidEl = $currentForm.find(shoptet.validator.invalidFieldClasses).first().parents('.form-group');
