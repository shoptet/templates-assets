...
(function (shoptet) {
  ...
  function trackOpenAiPixel(productData, formAction) {
    ...
    shoptet.scripts.signalCustomEvent('ShoptetOpenAiPixelTracked');
  }
  ...
  function handleAction(form, response) {
    ...
    if (priceId) {
      trackProducts(form, priceId, formAction, [
        ...
        shoptet.tracking.trackOpenAiPixel,
  ...
  function trackGtagCart(product, formAction, response) {
    ...
    const eventParams = {
      items: [createGtagItem(product, 0, findProductListing(product), createCartInfo(formAction, response, product))],
    };
