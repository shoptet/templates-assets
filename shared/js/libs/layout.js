...
(function(shoptet) {
    ...
    function ensure(value, condition) {
        if (condition(value)) {
            return value;
        } else {
            throw new Error(`[Validation Error]: Value must satisfy ${condition.name} condition`);
        }
    }
    ...
    function clearCache(key) {
        if (!Object.keys(state).includes(key)) {
            throw new Error(`[clearCache Error]: Invalid key. Key must be one of the following: ${Object.keys(state).join(', ')}`);
        }
        state[key] = undefined;
    }
