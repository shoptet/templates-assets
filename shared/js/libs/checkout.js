(function (shoptet) {
  ...
  function handleWithSidebar() {
    if (!compareHeight(shoptet.checkout.$checkoutContent, shoptet.checkout.$checkoutSidebar)) {
      fixSidebar();
    } else {
      shoptet.checkout.$checkoutSidebar.removeAttr('style');
    }
  ...
  function validateBillingForm(data, vatIdInput, isInitialCheck = false) {
    if (shoptet.config.onlineVatIdValidation && vatIdInput) {
      if (vatIdInput.value.trim() === '') {
        ...
        shoptet.validator.showValidatorMessage($('#vatId'), shoptet.messages['validatorVatIdWaiting'], 'msg-info');
      }
    }

    var shouldUpdateVatIdValidation =
      shoptet.config.onlineVatIdValidation && vatIdInput && vatIdInput.value.trim() !== '';
        ...
        try {
          ...
          if (shouldUpdateVatIdValidation) {
            if (response.getFromPayload('vatIdValidationStatus') === 2) {
              shoptet.validator.validatorMessage.hide($('#vatId'));
              shoptet.validator.showValidatorMessage($('#vatId'), shoptet.messages['validatorVatIdValid'], 'msg-ok');
          ...
          if (shouldUpdateVatIdValidation) {
            shoptet.validator.validatorMessage.hide($('#vatId'));
            shoptet.validator.showValidatorMessage($('#vatId'), shoptet.messages['validatorVatIdInvalid'], 'msg-error');
          }
        }
      },
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    $document.on('change', '.ordering-process #deliveryRegionId', function () {
      ...
      shoptet.cart.ajaxSubmitForm($parentForm.attr('action'), $parentForm[0], 'functionsForStep1', true, true);
    });
