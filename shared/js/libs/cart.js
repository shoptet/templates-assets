  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    $html.on('submit', 'form.pr-action, form.variant-submit', function (e) {
      ...
      if (shoptet.variantsCommon.handleSubmit($(this))) {
        ...
        var max = shoptet.helpers.toFloat($amount.attr('max')) || shoptet.config.defaultProductMaxAmount;
        var min = shoptet.helpers.toFloat($amount.attr('min')) || shoptet.helpers.resolveMinimumAmount(decimals);
        ...
        shoptet.cart.ajaxSubmitForm($this.attr('action'), $this[0], 'functionsForCart', 'cart', true);
      } else {
        shoptet.config.expressCheckoutAddToCart = undefined;
      }
    });
