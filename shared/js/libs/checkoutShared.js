...
(function (shoptet) {
    ...
    $.ajax({
      ...
      success: function (responseData) {
        try {
          ...
          shoptet.checkoutShared.shoptetPayPIS.countries = Object.entries(responseData).map(
          ...
          const bankCountTotal = shoptet.checkoutShared.shoptetPayPIS.countries.reduce(
            (acc, country) => acc + country.bankCount,
            0
          );
          ...
          shoptet.checkoutShared.shoptetPayPIS.paymentMethodPIS = document.querySelector(
            '.radio-wrapper[data-submethod="pis"]'
          );
          setDefaultCountry();
  ...
  function shoptetPayPISHandlePaymentMethodChange() {
    document.addEventListener('ShoptetBillingMethodUpdated', function (ev) {
      if (
        ev.target === shoptet.checkoutShared.shoptetPayPIS.paymentMethodPIS &&
    ...
    if (
      shoptet.checkoutShared.shoptetPayPIS.paymentMethodPIS &&
      shoptet.checkoutShared.shoptetPayPIS.paymentMethodPIS.classList.contains('active')
    ) {
      shoptet.checkoutShared.shoptetPayPIS.paymentMethodPIS
        .querySelector('.pisPaymentMethod')
        .classList.remove('pisPaymentMethod--hidden');
    }
  }
  ...
  function showPISModal() {
    ...
    shoptet.checkoutShared.shoptetPayPIS.countries.forEach(country => {
      ...
      const banksMessage =
        country.bankCount === 1
          ? shoptet.messages['PIScountryOptionOneBank']
          : shoptet.messages['PIScountryOptionMoreBanks'];
    ...
    showBanksByCountry(
      shoptet.checkoutShared.shoptetPayPIS.defaultCountry,
      clone.querySelector('.pisModal__bankSelection')
    );
    ...
    clone.querySelector('.pisModal__actions__confirm').addEventListener('click', function (e) {
      ...
      localStorage.setItem(
        'pisBankSelected',
        e.target.closest('.content-modal').querySelector('.pisModal__bankItem--active').getAttribute('data-bank-guid')
      );
  ...
  function updateBanks() {
    const banks =
      shoptet.checkoutShared.shoptetPayPIS.allPISbanks[shoptet.checkoutShared.shoptetPayPIS.defaultCountry].providers;
  ...
  function renderPIS(paymentMethodPIS) {
    ...
    let selected = Object.values(banks).find(
      bank =>
    ...
    if (shoptet.abilities.about.generation !== 3) {
      ...
      paymentMethodPIS.querySelector('.bankSelection__button').addEventListener('click', function (e) {
        showPISModal();
      });
    } else {
    ...
    shoptet.checkoutShared.shoptetPayPIS.paymentMethodPISdescription = paymentMethodPIS.querySelector(
      '.bankInformation__description'
    );
  }
        ...
        for (var i = 0; i < payuInputs.length; i++) {
          payuInputs[i].addEventListener('mousedown', function (e) {
            ...
            if (!activeLine.classList.contains('active')) {
              document.querySelector('#order-billing-methods .radio-wrapper.active input').checked = false;
  ...
  function handleGopayInstrumentSelection() {
    const selectedBillingCheckbox = ensure(
      document.querySelector('input[name="billingId"]:checked'),
      isHTMLInputElement
    );
  ...
  function getStatedValues() {
    shoptet.checkoutShared.deliveryCountryId =
      $('#deliveryCountryId').val() || shoptet.checkoutShared.deliveryCountries?.[0].id;
  ...
  function updatePrice(e) {
    ...
    if (e.detail.invalidate || e.detail.price.withVat === null) {
      ...
      if (!preserveValue) {
        var priceInputWithVat = e.target.querySelector('.external-shipping-method-price-with-vat');
        var priceInputWithoutVat = e.target.querySelector('.external-shipping-method-price-without-vat');
  ...
  function updatePriceSummary(shippingActive, billingActive) {
    ...
    var prices = {
      ...
      cart: {
        ...
        preauthorized: preauthorizedPrice
          ? shoptet.checkoutShared.getPriceFromElement(preauthorizedPrice, 'data-preauthorized-price')
          : undefined,
      },
    };
  ...
  function setTimeoutForExpiration(code, el, timeoutTime) {
    ...
    shoptet.checkoutShared.externalShippingDetails[code].timeout = setTimeout(function () {
      var ev = new CustomEvent('ShoptetExternalShippingExpired', {
  ...
  function setExternalShippingMethod(e) {
    ...
    var verificationCodeInput = e.target.querySelector('.external-shipping-method-verification-code');
  ...
  function setPickupPoint(data) {
    ...
    const shippingElement =
      document.getElementById('shipping-' + data.selectedShippingMethodId) ??
      document.querySelector('.radio-wrapper[data-id="shipping-' + data.selectedShippingMethodId + '"]');
    shippingElement.querySelector('.new-widget-choose').innerHTML =
      data.pickupPointWidget.name + ' ' + newLink.outerHTML;
  ...
  function setupDeliveryShipping() {
    ...
    $document.on('click', '.new-gen-choose a', function (e) {
      ...
      let href =
        '/action/NewGenWidget/Choose/?code=' +
        $parentsElement.find('[data-new-gen-code]').data('newGenCode') +
        '&deliveryCountryId=' +
        shoptet.checkoutShared.deliveryCountryId +
        '&shippingMethodId=' +
        shippingMethodId;
    ...
    $document.on('click', '.pickup-point-choose a', function (e) {
      e.preventDefault();
      let $parentsElement = $(this).closest('.radio-wrapper');
      let shippingMethodId = shoptet.checkoutShared.activeShipping
        .querySelector('input[data-guid]')
        .getAttribute('value');
      let deliveryCountryId = $('#deliveryCountryId').val();
      shoptet.checkoutShared.chooseABranchModal(
        '/action/PickupPoint/?&methodId=' +
          shippingMethodId +
          (isNaN(deliveryCountryId) ? '' : '&deliveryCountryId=' + deliveryCountryId),
      ...
      $document.on('submit', '#shipping-' + shippingMethodId + '-form', function (e) {
        ...
        $('#shipping-' + shippingMethodId + '-pickupPointId-selected').val(
          $('#shipping-' + shippingMethodId + '-pickupPointId option:selected').val()
        );
      ...
      $document.on('click', '#' + shippingMethodId + '-result .' + shippingMethodId + '-choose-button', function () {
  ...
  function twisto(twistoData) {
    ...
    function getAddress(prefix) {
      ...
      return {
        ...
        country:
          prefix === 'bill' && moreCountries ? $('#billCountryId option:selected').data('code') : deliveryCountryId,
        phones: [getPhone()],
      };
    }
    ...
    $('#submit-order')
      .closest('form')
      .submit(function (event, twistoVerificationSuccesfull) {
  ...
  function postDeliveryPoints(postDeliveryPointsData) {
    ...
    $(`${postDeliveryPointsData.deliveryPointPrefix}-form`).submit(function () {
      ...
      var postData =
        'zipCode=' +
        encodeURIComponent($.trim($(`${postDeliveryPointsData.deliveryPointPrefix}-wrapper .zip-code`).val()));
      postData +=
        '&postName=' +
        encodeURIComponent($.trim($(`${postDeliveryPointsData.deliveryPointPrefix}-wrapper .city`).val()));
      if (shipmentId) {
    ...
    for (var i = 0; i < elements.length; i++) {
      elements[i].addEventListener('mousedown', function (e) {
        ...
        if (this.getAttribute('data-external-script-code')) {
          ...
          shoptet.checkoutShared.callShippingBillingRelations();
          if (link) {
            if (link.classList.contains('zasilkovna-name') && !link.querySelector('.zasilkovna-default')) {
              return;
            }
